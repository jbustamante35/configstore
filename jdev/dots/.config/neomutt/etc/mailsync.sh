...
then
	...
	notify() { mpv --really-quiet ~/.config/neomutt/etc/notify.opus & pgrep -x dunst && notify-send -i ~/.config/neomutt/etc/email.gif "$2 new mail(s) in \`$1\` account." ;}
fi

echo " 🔃" > ~/.config/neomutt/.dl
...
do
	...
	newcount=$(find /media/jbustamante/voltron/Mail/"$account"/INBOX/new/ -type f -newer ~/.config/mutt/etc/.mailsynclastrun 2> /dev/null | wc -l)
	if [ "$newcount" -gt "0" ]
	then
...
touch ~/.config/neomutt/etc/.mailsynclastrun
