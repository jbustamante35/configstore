...
up_speed=`scp -P $ssh_port -v $test_file $ssh_server:$test_file 2>&1 | \
...
down_speed=`scp -P $ssh_port -v $ssh_server:$test_file $test_file 2>&1 | \
  grep "Bytes per second" | \
...
`ssh -p $ssh_port $ssh_server "rm $test_file"`
