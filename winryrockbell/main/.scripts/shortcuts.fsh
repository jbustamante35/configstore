...
# Output locations
shell_shortcuts="$HOME/.config/fish/.shortcuts"
...
(grep "source ~/.config/fish/.shortcuts"  $shellrc)>/dev/null || echo "source ~/.config/fish/.shortcuts" >> $shellrc
...
# directory shortcuts
sed "/^#/d" $folders | awk '{print "abbr "$1" \"cd "$2" ; and ls -a\""}' >> $shell_shortcuts
...
# dotfile shortcuts
sed "/^#/d" $configs | awk '{print "abbr "$1" \"$EDITOR "$2"\""}' >> $shell_shortcuts
